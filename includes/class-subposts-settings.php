...
final class Settings {
	 ...
	 *
	 * Prefixes option name with "subposts_"
	 * and the page name with "subposts-".
	 *
	 * @since 1.0.0
	 *
	...
	public static function add_field( $field, $options, $page, $section = 'default' ) {
		...
		// Handle prefixing the name with subposts_options
		if ( preg_match( '/([^\[]+)(\[.+\])/', $field, $matches ) ) {
			$id = "subposts_" . trim( preg_replace( '/[\[\]]+/', '_', $field ), '_' );
			$name = "subposts_options[{$matches[1]}]{$matches[2]}";
		} else {
			$id = "subposts_{$field}";
			$name = "subposts_options[{$field}]";
		}

		// Build the callback arguments
		$class = sanitize_key( $field );
		$args = array(
			'class'     => "slug-settings-field slug-settings-{$page}-field subposts_{$class}-field",
			'option'    => $field,
			'id'        => $id,
			'name'      => $name,
			'label'     => $options['label'],
