...
final class Registry {
	...
	public static function get( $option, $default = null, $true_value = false, &$has_override = null ) {
		...
		if ( ! self::has( $option ) ) {
			trigger_error( "[Subposts] The option '{$option}' is not supported.", E_USER_NOTICE );
		}
	...
	public static function load( $reload = false ) {
		...
		// Load the options
		$options = get_option( 'subposts_options' );
	...
	public static function save( $what = true ) {
		update_option( 'subposts_options', self::$options );
	}
}
